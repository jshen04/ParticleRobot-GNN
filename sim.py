...
class Simulator:
    def __init__(self, num, start, goal, centralized=False, visualizer=None):
...
class GraphSimulator(Simulator):
    def __init__(self, num, start, goal, centralized=False, visualizer=Visualizer()):
        super().__init__(num, start, goal, centralized, visualizer)
    ...
    def get_label(self):
        ...
        #     labels.append([particle.body.velocity[0], particle.body.velocity[1]])
        # return torch.tensor(labels, dtype=torch.float)
        for particle in self.superagent.particles:
            if np.linalg.norm((particle.currCOM[0] - self.goal[0], particle.currCOM[1] - self.goal[1])) < np.linalg.norm((particle.prevCOM[0] - self.goal[0], particle.prevCOM[1] - self.goal[1])):
                labels.append([0, 1])
            else:
                labels.append([1, 0])
        return torch.tensor(labels, dtype=torch.float)
        # if np.linalg.norm((self.superagent.currCOM[0] - self.goal[0], self.superagent.currCOM[1] - self.goal[1])) < np.linalg.norm((self.superagent.prevCOM[0] - self.goal[0], self.superagent.prevCOM[1] - self.goal[1])):
        #     return torch.tensor([0, 1], dtype=torch.float)
        # else:
        #     return torch.tensor([1, 0], dtype=torch.float)

    def get_super_label(self):
        '''
        :return: 2 x n tensor s.t. [0, 1] means system is getting closer, [1, 0] means getting further
        '''
        # labels = []
    ...
    def step(self, action):
        ...
        # label = final_obs[:, [2, 3]]
        # if self.centralized:
        #     label = self.get_super_label()
        # else:
        #     label = self.get_label()
        label = self.get_label()
        superlabel = self.get_super_label()
        ...
        superdata = torch_geometric.data.Data(x=node_features, edge_index=edge_index, y=superlabel)

        return data, superdata
...
#             x = self.world.object.body.position[0]
#             y = self.world.object.body.position[1]
#             xdot = self.world.object.body.velocity[0]
#             ydot = self.world.object.body.velocity[1]
#             obs = np.concatenate([obs, [x, y, xdot, ydot]])
#         obs = scale(obs)
#         return np.array(obs)
#
...
#             action = self.decimal2binary(action)
...
#     # Helper functions
#
#     def decimal2binary(self, decimalAction):
#         '''
